...
namespace HL7.Dotnetcore
{
    ...
    public class HL7Encoding
    {
        ...
        public void EvaluateSegmentDelimiter(string message)
        {
            ...
            {
                if (message.Contains(d, StringComparison.Ordinal))
                {
                    this.SegmentDelimiter = d;
                    return;
                }
            }
        ...
        public string Encode(string val)
        {
            ...
            for (int i = 0; i < val.Length; i++)
            {
                ...
                {
                    // All other non-visible characters will be preserved
                    string v = string.Format(CultureInfo.CurrentCulture, "{0:X2}", (int)c);
