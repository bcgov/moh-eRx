...
accessToken=$(curl --location --request POST ${tokenEndpoint} \
  ...
  --data-urlencode "client_secret=${HEALTH_CHECK_CLIENT_SECRET}" 2> /dev/null | jq -r '.access_token')

# submit a transaction to the current service
curl --request POST 'https://127.0.0.1:8080/' \
  --header 'Content-Type: application/json' \
  --header "Authorization: Bearer ${accessToken}" \
  --data "{'resourceType':'DocumentReference','status':'current','date':'$(date --iso-8601=seconds)','content':[{'attachment':{'contentType':'x-application/hl7-v2+er7','data':'${payload}'}}]}" > /dev/null
